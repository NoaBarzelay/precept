...
**Validity is bi-temporal.** Every entry records when it became true and when it stopped, separately from when it was recorded and when it was superseded. Retirement invalidates; removal tombstones and purges. This carries R1.9, R1.10, R2.3, R2.8, R2.9, R2.10, and makes N8's two grains fall out of the data model.
...
**Format is versioned from the first entry.** Cards and the operational database each carry a schema version. Additive changes migrate forward through a versioned, transactional migration chain, so a failed migration is retryable, not half-applied. A file written by a newer major version is refused, not opened, because the alternative is silent destruction on the next write [N12].
