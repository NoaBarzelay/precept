"""CLAUDE_MD artifact (#3): a kept SOFT convention is written into a Precept-owned
...
from precept import claude_md, compile as compile_mod, install
...
def _conv(
    ...
    le = Lesson(
        ...
        artifact_type=ArtifactType.CLAUDE_MD, determinism=Determinism.STYLISTIC,
...
def test_target_global_is_user_rules_file(isolated):
    dest = claude_md.target_for(_conv("g", "prefer composition"))
    assert dest == isolated / "rules" / "precept-conventions.md"


def test_target_repo_is_project_rules_file(isolated, tmp_path):
    root = tmp_path / "proj"
    dest = claude_md.target_for(_conv("r", "x", scope=Scope.REPO, scope_value=str(root)))
    assert dest == root / ".claude" / "rules" / "precept-conventions.md"


def test_target_repo_without_root_is_unplaceable(isolated):
    assert claude_md.target_for(_conv("r", "x", scope=Scope.REPO)) is None


def test_target_language_is_marked_file(isolated):
    dest = claude_md.target_for(_conv("l", "type hints", scope=Scope.LANGUAGE, scope_value="python"))
...
def test_is_managed_excludes_bootstrap_pending_and_nonconvention(isolated):
    assert claude_md.is_managed(_conv("ok", "x")) is True
    # imported from the user's own CLAUDE.md -> already in context, never re-emitted
    assert claude_md.is_managed(_conv("b", "x", origin=Origin.BOOTSTRAP)) is False
    assert claude_md.is_managed(_conv("p", "x", status=Status.PENDING)) is False
    rule = _conv("rule", "x")
    rule.artifact_type = ArtifactType.RULE
    assert claude_md.is_managed(rule) is False


# --- rendering -------------------------------------------------------------
def test_render_is_sorted_and_headed(isolated):
    text = claude_md.render_file([_conv("z", "second"), _conv("a", "first")])
...
def test_render_language_emits_paths_frontmatter(isolated):
    text = claude_md.render_file([_conv("l", "type hints")], globs=["**/*.py"])
...
def test_write_creates_file_with_convention(isolated):
    claude_md.write_managed_rules([_conv("g", "prefer composition over inheritance")])
    dest = isolated / "rules" / "precept-conventions.md"
    assert dest.exists()
    assert "prefer composition over inheritance" in dest.read_text(encoding="utf-8")
    assert [p.name for p in claude_md.managed_files()] == ["precept-conventions.md"]
...
def test_recompile_without_lesson_removes_managed_file_only(isolated):
    ...
    claude_md.write_managed_rules([_conv("g", "x")])
    assert (isolated / "rules" / "precept-conventions.md").exists()

    claude_md.write_managed_rules([])  # lesson archived/deleted -> recompile empties it
...
def test_strip_all_is_exact_inverse(isolated):
    claude_md.write_managed_rules([_conv("g", "x"), _conv("l", "y", scope=Scope.LANGUAGE, scope_value="go")])
    assert claude_md.managed_files()
    claude_md.strip_all()
    assert claude_md.managed_files() == []
...
def test_compile_all_writes_convention_without_changing_policy_count(isolated):
    # A CLAUDE_MD lesson is SOFT: it must NOT enter the hook count compile_all returns,
