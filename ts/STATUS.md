...
As of this writing: **216 tests passing, offline; `tsc --noEmit` clean; the dependency-rule and interception fitness functions green; CI green.**
...
| Detect cost gate | `infer/prefilter`, `infer/detect`, `record/proposed` | A cheap, recall-biased pre-filter before the model (Risk 5, R1.18): corrections/silent-edits/stated-knowledge/agent-research always propose; a plain instruction proposes only on durable cues (always/never/use/prefer/...), so a one-off task turn spends no call. Filtered evidence stays in the append-only log for the R1.14 hindsight pass. `detect` returns queued/proposed/filtered/alreadyProposed counts. A second gate, the append-only proposal ledger (`record/proposed`), records every window sent to the model: the evidence log has no cursor and is read whole on every run, so without it each run re-pays for the entire history and the queue fills with re-worded duplicates of one window. The ledger records the send, not the outcome, because an abstention (the common result) leaves no trace in the queue or the decision log. A filtered window stays out of the ledger: the pre-filter is free to re-run and R1.14 wants a second look at it. |
...
| Session-start review prompt | `record/backlog`, `injection` | SessionStart was a no-op (R1.8); it now reports the review queue and asks for an interactive walk. The queue is the one step needing a human and nothing asked for it, so candidates accumulated unreviewed while the catalog stayed empty. Bounded (fixed summary plus three truncated previews, so a queue of 3 and of 300 cost the same context) and throttled (once per `PRECEPT_BACKLOG_INTERVAL_HOURS`, default 24, 0 disables). Stamps only when it actually injects. |
| Knowledge in the vault | `store/vault`, `store/card` (routing), `gate`, cli `keep --folder`/`rescan` | Split by kind: a convention is an instruction for the agent and stays a card under the catalog root; knowledge is for Noa and commits as a note in her Obsidian subject tree, in her own contract (`type: knowledge`, `updated`, title/date/topic/purpose/tags, trailing `## Sources`). Precept's typed fields ride in one nested `precept:` key, which is what makes the note round-trip and what marks it as Precept's: a note without that block is hers, and `parseNote` refuses it, so Precept can neither adopt nor overwrite her writing. Writes refuse to clobber a non-Precept file or a different entry's note; `rescan` repairs the id-to-path map after she moves notes in Obsidian. `PRECEPT_VAULT` unset falls back to cards, so the system runs unconfigured. |
| Noa's own notes, read-only | `store/vault` (`readExternalNotes`), `retrieve/index` (`source` column), `retrieve/retrieve` | Retrieval covers the whole vault, not just Precept's slice of it: 584 of her own `type: knowledge` notes are indexed section-granular alongside 95 governed entries. Strictly read-only, and structurally so: `readExternalNotes` returns an `ExternalDoc` (path, title, content) with no id, version, status or validity, so one cannot reach `writeCard` or any lifecycle path. `Claude/` and `Claude Conversations/` are excluded (already in context, and Precept's own past output). Hits carry a `source`, and injection labels hers as hers rather than as a recorded rule. Ranking gives each source a guaranteed foothold and then lets relevance decide: an earlier governed-first reservation put entries scoring 6.0 ahead of the correct section scoring 33.5, which was worse than no reservation. Rebuild is 7s over 13MB of vault; the index is ~21MB of local derived state. |
| Periodic vault refresh | `record/refresh`, `retrieve/index` (`refresh`), `observation` (SessionEnd), cli `refresh` | Her notes drift whenever she writes in Obsidian, which produces no evidence and no tool calls, so nothing in the loop would notice. SessionEnd now spawns a detached `refresh` on a throttle (`PRECEPT_REFRESH_INTERVAL_MINUTES`, default 30, 0 every session). Not gated on the inference backend, since it spends no tokens, and deliberately not gated on new evidence, since her editing is exactly the change that generates none. Incremental against a size/mtime manifest: 0.18s when nothing changed, 0.24s for one edited note, against 4.3s for a full rebuild. Writes run in one transaction; per-statement commits made the cold path 36s. |
