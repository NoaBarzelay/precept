...
The full test suite is 231 functions and runs offline and hermetic. Every LLM call is injectable through a `FakeClient` at each seam, and the test harness redirects state to temp directories so no test touches the real `~/.claude`. An enforcement tool you cannot test deterministically is one you cannot trust, so testability was a design constraint from the start, not a cleanup pass at the end.
