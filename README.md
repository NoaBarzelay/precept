...
|---|---------|-------------|-------------|
| N1 | Reliability | A fault in Precept lets the work continue rather than blocking the session, and is recorded against the preference that did not hold, so a preference that stops holding surfaces rather than passing for a quiet session. *Driver: Risk 2, Risk 1.* | Fail-open fault-record test |
...
| N3 | Safety | Model-authored logic executes only as data, never as code, so an artifact the model produced degrades to no effect rather than to an executed action. *Driver: R1.17, Risk 1.* | Logic-as-data test |
| N4 | Security | Precept reaches beyond the machine at one boundary only, which the mechanically checked path never crosses. Closing that boundary leaves every mechanically checkable preference enforcing. *Driver: US5, Risk 5, R1.17, R1.18.* | Single-egress boundary test |
| N5 | Integrity | A preference enforces deterministically only where its enforcement can be validated at the moment it is recorded, and otherwise steers without enforcing, so nothing claims enforcement it cannot deliver. *Driver: Non-Goal 2, R1.17, Risk 3.* | Unenforceable-claim rejection test |
| N6 | Auditability | Every enforced decision and every recorded entry resolves to its origin: the session excerpt it came from, the approval it passed, and the basis it was decided against. *Driver: Risk 1, Risk 2, US2.* | Provenance-resolution test |
| N7 | Accountability | No change reaches enforcement until it is approved, so no change to the agent's behavior originates with Precept alone. *Driver: US5, R1.5, R1.15, R1.16, Risk 1.* | Unapproved-change rejection test |
| N8 | Reversibility | Precept's changes are undoable at two grains: a retired preference stays recoverable, a removed one is destroyed, and removing Precept entirely returns everything it changed to its prior state. It changes only what it owns, so anything authored alongside it is left as it was. *Driver: Risk 2, Non-Goal 1, US5, R1.16.* | Full-undo round-trip test |
| N9 | Capacity | What Precept injects into a turn does not grow with the amount it has recorded, and stays within its stated size and relevance bounds. *Driver: Risk 4, Risk 3, R2.5, R2.7.* | Injection-bound test |
| N10 | Cost | Precept's model spend is measured and attributed to the work that incurred it, and raises an alert when it crosses its stated threshold; work not required to complete a turn runs off that turn. *Driver: Risk 5, O1.* | Cost-attribution test |
| N11 | Privacy boundary | Learned content lives outside this repository, and no recorded entry, local state, or personal marker is ever tracked in it. *Driver: Non-Goal 5.* | Tracked-private-artifact test |
