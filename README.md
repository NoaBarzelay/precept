...
|---|---------|-------------|-------------|
| N1 | Reliability | A Precept fault degrades to no enforcement, never to a blocked session: an error, a missing key, or an unreadable cache on a guarded call resolves to allow. Because failing open is silent, a flow that cannot reach its model records that it could not, so an inert loop surfaces as a fault rather than passing for a quiet session. That stance covers the enforcement plane only. The learning plane takes the opposite one on purpose, so a detection error records nothing rather than a wrong preference, because a missed lesson is recoverable by repeating myself and a wrong one is not. *Driver: Risk 2 (a wrong enforcement is the costly error); Risk 1; R1.2.* | Fault-injection test at every hot-path seam; unreachable-backend diagnostic test; detection-fails-closed test |
| N2 | Performance efficiency | The deterministic enforcement path makes no model call and reaches no network in any configuration, and its worst case is bounded by construction: one fresh process per guarded call, and any pattern match abandoned at a wall-clock bound. The turn-end judgment path (R1.17) calls a model by design, and that cost belongs to N10. *Driver: Risk 5 (latency I feel is what makes me set it aside); R1.16.* | No-model-call and no-network-import test on the enforcement path; pattern-bound abandonment test; per-call added-latency budget test |
| N3 | Safety | Model-authored logic executes only as data through a fixed interpreter, never as code. A pattern is rejected at compile time if catastrophic and abandoned at runtime under a wall-clock bound that resolves to no match; nested inference is recursion-guarded. A model-generated artifact that fails degrades to inert, not to dangerous. *Driver: R1.16 (a preference compiles to a matcher that then runs on my machine); Risk 1 (the model can misread what I meant).* | Source-level logic-as-data test (no dynamic execution on the enforcement path); catastrophic-pattern rejection test; runtime-bound abandonment test |
| N4 | Security and bounded autonomy | The enforcement plane sends nothing off the machine and holds no credential. The learning plane's model access is the only egress, and a single control disables that loop entirely while already-recorded preferences keep enforcing, so the ceiling on how much of this runs is mine to set. Local-first is the default, not a setting. The control bounds how much the loop runs; N7 governs what a running loop may commit. *Driver: user story "stay in control of how my agents behave"; Risk 5 (a ceiling I control).* | No-egress test on the enforcement path; learning-loop disable-control test |
| N5 | Integrity | An entity cannot claim enforcement it cannot deliver. A hard tier on an event that cannot block a call is a construction-time error rather than a runtime surprise, and a preference earns deterministic enforcement only by compiling to a matcher that passes a typed validator; otherwise it stays steering-only. A compiled matcher is deterministic by construction, the same preference against the same call yielding the same decision on every run. The point against Risk 3 is not more enforcement but a loud failure: a preference that cannot be enforced says so at construction instead of silently not firing. *Driver: Non-Goal 2 (the hard set stays small on purpose); R1.16; Risk 3.* | Unenforceable-tier construction-time failure test; matcher-validator gate test; repeated-run determinism test |
| N6 | Auditability | Every enforced decision and every recorded entry resolves to its origin: the session excerpt it came from, the provenance gate it passed, and, for a preference enforced by judgment, the verdict prompt held on the entry itself. Every enforcement appends a decision record. Nothing enforces anonymously. *Driver: Risk 1 (a preference can only be judged if its origin is on the record); Risk 2 (a wrong enforcement must name its rule and its basis, which is what makes override possible); user story "trust its output instead of re-checking".* | Provenance-resolution test; decision-record append test |
| N7 | Accountability | Precept has no autonomous write path to my agent's behavior. Every behavioral change and every catalog entry originates from a proposal that was approved. The functional requirements specify the gate; this is the guarantee that there is no path around it. *Driver: user story "my setup does not drift or change without me"; R1.5, R1.14, R1.15; Risk 1.* | Structural approval-gate test (no unapproved entry reaches enforcement by any path) |
| N8 | Reversibility and containment | Adopting Precept is undoable at two grains. One preference: a single command retires it, archived rather than deleted, so the undo is exact and recoverable. The whole install: every commit target is backed up before it is touched, writes are atomic, and uninstall restores the tree. Precept writes only what Precept owns, so configuration I authored by hand is written around rather than through, and survives both install and uninstall. *Driver: Risk 2 (one-command override is what makes a wrong enforcement survivable); Non-Goal 1; user story "stay in control".* | Install and uninstall round-trip test; foreign-configuration preservation test; retired-entry recoverability test |
| N9 | Capacity | The size of what Precept injects per turn stays flat as the catalog grows, and the entry a turn needs stays inside the injected slice rather than falling below the cutoff. Every set it injects is bounded by an entry count and by a total size, and nothing enters below a relevance floor. I will not fix the numeric bar before the evaluation that settles it, because a bar set from today's small corpus would encode the status quo as the requirement (Open Question 1). *Driver: Risk 4 (the corpus decaying as it accumulates); Risk 3; R2.5, R2.7.* | Injected-slice cap test; retrieval precision and latency benchmark at ten times current catalog size |
| N10 | Cost | The learning loop's spend and its added interactive latency are measured and attributable per flow, and each is bounded by a threshold that raises an alert on its own. The costly passes run off the interactive turn, and a deterministic relevance test decides whether a turn warrants a model call before one is made. No target number is fixed until a usage baseline exists to set it from. *Driver: Risk 5; O1 (a loop I abandon compounds nothing).* | Per-flow cost report; off-turn placement test; added-latency measurement against threshold |
| N11 | Privacy boundary | Learned content (the catalog, local state, any vault) lives outside this repository, and the boundary is enforced rather than asserted: no populated entry, local session config, or personal marker (home paths, phone patterns, vault mounts) is ever tracked. The public code plane never contains my private data plane. *Driver: Non-Goal 5 (this is published as-is, to be read and copied from).* | Tracked-private-artifact test, CI-gated |
| N12 | Data durability | Preferences and knowledge accumulated under an earlier version of Precept survive an upgrade rather than stranding, so what I taught it once outlives the format it was first stored in. *Driver: O1 key result (what I convey stays embedded) and O2 key result (knowledge compounds instead of being re-learned), both of which fail if an upgrade orphans the catalog.* | Cross-version migration round-trip test |
