...
4. **Behaviors and information become hard to track and use as they accumulate** (impact: medium, probability: high). As the information retained in catalog and the enforced behaviors keep compounding, usability and recall can degrade due to problems such as retrieval precision falling as the index grows, and too much context being injected so that even a retrieved entry loses attention (a model's adherence to any single instruction weakens as the number of instructions in context rises). Where risk 3 is one convention losing out within a single session, this is the corpus as a whole decaying as it accumulates, which makes that failure steadily more frequent and harder to diagnose.
   ...
   *Mitigations:*
   ...
   - **Budgeted always-on core:** the always-loaded set is capped and other artifacts load only when conditions match; most of the catalog therefore stays out of context, and accumulation cannot silently push standing context past the range where adherence holds. [R1.4, R1.5]
   - **Identify and merge dups on creation:** governance above prunes duplicates once they exist; the root fix is to avoid minting them. A candidate that matches an existing entry would update that entry instead of creating a parallel one, so the catalog trends toward one entry per preference rather than a drift of near-copies to reconcile later. [would extend R3.1, R3.3]
   - **A budget on the slice Precept injects, not just the always-on set:** the cap above bounds what loads natively, but Precept's hook also retrieves and injects a per-prompt slice of its own, the relevant knowledge notes and the conventions it injects by relevance, as `additionalContext`, and that slice can grow long and low-precision as the index grows. Because Precept builds this slice itself, capping how many entries it injects per turn and setting a minimum relevance score is deterministic selection on its own retrieval path. What Claude Code retrieves natively (CLAUDE.md, the scoped rule files, skills matched by description) is not capped here; that is governed by the budget and scoping above and by keeping the corpus small (dedup, ranking, consolidation). [would extend R2.2]
   - **Rank by precision signals, not lexical overlap alone:** at scale, keyword score cannot separate the one relevant rule from many generic near-matches. Combining it with scope-specificity (a rule scoped to this exact path or repo outranks a global one), recency, and how many times the rule has been confirmed would lift the precise, live entry back above stale generic ones and into the top-k (an embedding index is one option here). [would extend R2.2; Open Question 1]
   - **Periodic consolidation of the catalog:** beyond retiring entries one at a time, a scheduled compaction would merge clusters of overlapping conventions into a single canonical entry. [would extend R3.3]

5. **The Claude Code contract changes underneath Precept** (impact: medium, probability: medium). Precept rides Anthropic's hook, permission, and exit-code surface, which is unversioned and has changed before. Every hard rule leans on two halves of that surface holding: the inbound event Precept reads (the tool name, its arguments, the working directory, the stop-hook transcript) and the outbound decision it emits (the deny the host is supposed to honor). It can fail by (1) inbound drift: the event still fires and the hook still exits cleanly, but a renamed or re-nested field makes the rule read nothing, so the guard resolves to allow (2) outbound drift: the schema or exit convention for signaling a block changes, so the host ignores a deny Precept did decide (3) a hook event or permission rule is renamed, removed, or narrowed, so a guard registered against it never fires. Because the runtime fails open (N1), each of these downgrades a hard rule to a silent no-op: no error, no crash, enforcement simply stops while I keep trusting it. The fix is small and localized; the cost is the window before I notice.
   Example: a host update renames the field the pre-tool event carries the tool's arguments in. The block rule still loads, the hook still runs and exits cleanly, but it now reads an empty argument set, matches nothing, and the command it was written to stop runs untouched, with nothing to tell me the guard went dead.

   *Mitigations:*
   - **Live-fire the enforcement path, not just its wiring:** at startup and whenever the host version changes, drive a known-blocked action through the real hook and permission surfaces end to end and assert it is still denied, so a block that no longer fires is surfaced loudly instead of trusted. The engine is already checked against committed cases in CI, but that proves only that Precept decided to deny; only running the action through the live host proves the host still honors that decision, which is exactly the seam a contract change opens. [would extend N1]
   - **Validate the inbound event and record what it cannot read:** each hook run checks the event against the shape it expects, and one it cannot parse (a missing or renamed tool field, an unfamiliar transcript) still fails open but is recorded as a health signal rather than passing as a silent allow, so inbound drift surfaces as a logged anomaly a diagnostic can report instead of a quiet gap. [would extend N1]
   - **Build only on documented contract points:** Precept emits the published hook decision schema and the documented permission surface rather than any incidental behavior, and every entity type is specified against the verified host contract before it is built, not inherited from a doc that may have moved. Documented surfaces still change, but they change visibly; undocumented ones change without notice. [R1.2]
