...
Each one is derived rather than picked off a checklist: every entry names the objective, user story, risk, or requirement it descends from, and each of the five risks above is answered here by at least one property. Each is stated as a property with the condition it holds under, the method that would catch a violation, and a status: **built** (holds and is asserted in CI), **partial** (part holds or part is asserted, the rest is specified), **designed** (specified, not implemented), **planned** (roadmapped, not specified). As in the functional requirements, the property is the requirement and the mechanism is in the verification column.

The frame is ISO/IEC 25010:2023, with one correction. That model puts accountability under security and does not name auditability, provenance, or human oversight at all, which are the properties a system that learns from me and rewrites my configuration lives or dies on. So N6 and N7 are first-class attributes here rather than security sub-characteristics, following the "accountable and transparent" characteristic in NIST's AI Risk Management Framework, which exists for that gap.

The table is ordered by what I would defend first. N1 and N7 are the two I would not trade: a tool that blocks me, or that changes my setup behind my back, is a tool I stop running, and stopping forfeits every other property below. N2 through N6, and N8, are what make the loop safe to leave on. N9 through N12 are what decide whether it survives a year of accumulation.

| # | Quality | Requirement | Verified by | Status |
|---|---------|-------------|-------------|--------|
| N1 | Reliability (fault tolerance) | A Precept fault degrades to no enforcement, never to a blocked session: an error, a missing key, or an unreadable cache on a guarded call resolves to allow. Failing open is silent, so it has a second half: every learning flow records when it cannot reach a model, and a diagnostic reports an unreachable backend, so a dead loop surfaces as a fault instead of passing for a quiet session (an expired credential once left it dead for weeks). That stance covers the enforcement plane only. The learning plane takes the opposite one on purpose: detection fails closed, so an error there records nothing rather than a wrong preference, because a missed lesson is recoverable by repeating myself and a wrong one is not. *Driver: Risk 2 (a wrong enforcement is the costly error); Risk 1; R1.2.* | Fault-injection tests at every hot-path seam (unreadable cache, malformed event, model error) assert an allow decision; a test asserts a flow that cannot reach a model records the failure while still failing open; a test asserts a detection error records nothing. | built |
| N2 | Performance efficiency (time behaviour) | The deterministic enforcement path (R1.16) makes no model call in any configuration, and its worst case is bounded by construction: one fresh process per guarded call, and any pattern match abandoned at a wall-clock bound. The turn-end judgment path (R1.17) does call a model by design; that cost is N10's, not this one's. Keeping the deterministic path free of third-party imports is the implementation choice that holds this true, not the requirement. *Driver: Risk 5 (latency I feel is what makes me set it aside); R1.16.* | A test asserts the deterministic stop path leaves the model layer unimported (`precept.judge` and `anthropic` absent from `sys.modules` after a guarded call); the pattern bound (1.0 s) is asserted to return well inside it rather than run to completion. Not yet asserted: end-to-end added latency per guarded call, and the absence of any network-capable import (the current assertion covers the model layer, which is narrower than the claim). | partial |
| N3 | Safety (fail-safe) | Model-authored logic executes only as data through a fixed interpreter, not as code. A pattern is rejected at compile time if catastrophic and abandoned at runtime under a wall-clock bound that resolves to "no match"; nested inference is recursion-guarded. A model-generated artifact that fails degrades to inert, not to dangerous. *Driver: R1.16 (a preference compiles to a matcher that then runs on my machine); Risk 1 (the model can misread what I meant).* | Adversarial tests: a catastrophic pattern is rejected at compile and abandoned under bound at runtime; a fork-bomb-class correction cannot compile to executable code. Not yet asserted: a source-level check that no module on the enforcement path calls `eval` or `exec`, which is what would make "only as data" a structural invariant instead of a sampled one. | partial |
| N4 | Security (confidentiality) and bounded autonomy | The enforcement plane sends nothing off the machine and holds no credential. The learning plane's model access is the only egress, and one environment variable disables that loop entirely while already-learned rules keep enforcing, so the ceiling on how much of this runs is mine to set. Local-first is the default, not a setting. (The flag bounds how much the loop runs; N7 governs what a running loop is allowed to commit.) *Driver: user story "stay in control of how my agents behave"; Risk 5 (a ceiling I control).* | A test asserts the enforcement path does not import the model layer (shared with N2, and narrower than "no egress": no test yet pins the absence of a network-capable import); a test asserts the learning loop is inert when the disable flag is set. | partial |
| N5 | Integrity | An entity cannot claim enforcement it cannot deliver. A hard tier on an event that physically cannot block a call is a construction-time error, not a runtime surprise, and a preference earns deterministic enforcement only by compiling to a matcher that passes a typed validator; otherwise it stays steering-only. The point against Risk 3 is not more enforcement but a loud failure: a preference that cannot be enforced says so at construction instead of silently not firing. A compiled matcher is also deterministic by construction, the same rule against the same call yielding the same decision on every run, which is what makes the enforcement scorecard a measurement rather than a sample. *Driver: Non-Goal 2 (the hard set stays small on purpose); R1.16; Risk 3.* | A type-level test asserts a hard tier on a non-blockable event raises. | built |
| N6 | Auditability | Every enforced decision and every learned entity resolves to its origin. `precept why` shows the session excerpt a preference came from and the provenance gate it passed (user-typed turns only); every policy match appends a decision-log line; the verdict prompt for a judgment rule is stored on the entity's own card. Nothing enforces anonymously. *Driver: Risk 1 (a preference can only be judged if its origin is on the record); Risk 2 (a wrong enforcement must name its rule and its basis, which is what makes override possible); user story "trust its output instead of re-checking".* | Tests assert a match appends a well-formed log line and that `why` resolves provenance; the same log is the source of the live fire counts governance reads. | built |
| N7 | Accountability (human oversight) | Precept has no autonomous write path to my agent's behavior. Every behavioral change and every catalog entry originates from a proposal that was approved. The functional requirements specify the gate; this is the guarantee that there is no path around it. *Driver: user story "my setup does not drift or change without me"; R1.5, R1.14, R1.15; Risk 1.* | The gate is structural rather than sampled: compilation is the single point where a recorded preference becomes an enforced policy, and it yields nothing for an entry that is not approved, so every writer downstream of it is gated by construction. Tests exercise both sides of that gate (an approved entry compiles and enforces, a pending one produces no policy). | built |
| N8 | Reversibility and containment | Adopting Precept is undoable at two grains. One preference: a single command retires it, archived with a back-pointer rather than deleted, so the undo is exact and recoverable. The whole install: every commit target is backed up before it is touched, writes to `~/.claude` are atomic (temp in the same directory, fsync, rename), and uninstall restores the tree. Precept writes only what Precept owns, so configuration I authored by hand is written around rather than through: it sits alongside Precept's own artifacts in the same targets and survives both install and uninstall (Non-Goal 1). *Driver: Risk 2 (one-command override is what makes a wrong enforcement survivable); Non-Goal 1; user story "stay in control".* | Install and uninstall round-trip tests assert the tree returns to its pre-install state byte for byte; a test asserts a foreign hook and foreign settings keys survive an install unchanged; a test asserts a retired entry stays recoverable. | built |
| N9 | Capacity (quality under growth) | The size of what Precept injects per turn stays flat as the catalog grows, and the entry a turn needs stays inside the injected slice rather than falling below the cutoff. Today each retrieved set is bounded by a top-k, and the knowledge slice also by a character cap; the concatenation of every set Precept injects has no single ceiling yet, and there is no relevance floor below which it injects nothing. I will not set the numeric bar before the evaluation that would settle it runs, because a bar set from today's small corpus would encode the status quo as the requirement (Open Question 1). *Driver: Risk 4 (the corpus decaying as it accumulates); Risk 3; R2.5, R2.7.* | Today: asserted caps on query terms (24), on retrieved-set size (top 5 per set), and on the knowledge slice length (2,000 characters). Specified, not built: a benchmark over a synthetic catalog an order of magnitude larger than mine, reporting recall at k, total injected size, and p95 retrieval latency. | partial |
| N10 | Cost | The learning loop's spend and its added interactive latency are measured and attributable per flow, so the overhead is visible while it is still cheap to change. The costly passes run off the interactive turn (detection runs once at session end, not per turn), and a deterministic pre-filter decides whether a turn warrants a model call before one is made. Not in place: the measured per-turn added latency, and a threshold on either dimension that raises an alert on its own. No target number is set, because the usage baseline that would justify one does not exist yet (see KPIs). *Driver: Risk 5; O1 (a loop I abandon compounds nothing).* | `precept tokens` reports the fixed per-flow prompt overhead, currently 996 tokens for a detection pass, 1,578 for a compile, 691 for a capture, and 177 to 393 for a verdict. Those are offline estimates and no observed per-session baseline is committed yet, which is the gap between reporting the cost and knowing it. The off-turn placement of detection is asserted in the hook tests. The latency measure and the alert thresholds are specified only. | partial |
| N11 | Privacy boundary | Learned content (the catalog, local state, any vault) lives outside this repository, and the boundary is enforced, not asserted: a CI test fails the build if a populated catalog card, local session config, or personal marker (home paths, phone patterns, vault mounts) is ever tracked. The public code plane never contains my private data plane. *Driver: Non-Goal 5 (this is published as-is, to be read and copied from).* | `tests/test_repo_privacy.py` runs in CI and fails on any tracked private artifact. | built |
| N12 | Data durability (upgradeability) | Preferences and knowledge accumulated under an earlier version of Precept survive an upgrade rather than stranding, so what I taught it once outlives the format it was first stored in. Nothing here is built: N8 covers a clean uninstall, not the forward migration of months of accumulated entries when their format changes. *Driver: O1 key result (what I convey stays embedded) and O2 key result (knowledge compounds instead of being re-learned), both of which fail if an upgrade orphans the catalog.* | Specified only: a round-trip test that writes entries in the previous format, migrates, and asserts every entry is still readable and semantically unchanged. | planned |

**The trade-off I made on purpose.** N1 (fail open) and N5 (no unearned enforcement) both spend enforcement coverage to buy non-interference, and Non-Goal 2 is the same choice stated as scope. That is deliberate: a false block gets the tool turned off, which forfeits every other property here, so I would rather under-enforce and know it than over-enforce and be trusted less. N6 and N7 are what keep that from becoming an excuse, since an enforcement that does not fire is still on the record and nothing enforces without my approval in the first place.

**What is deliberately absent.** No availability SLA, no horizontal-scalability target, no multi-tenant isolation, and no portability requirement. There is one user on one machine, the system is allowed to be absent by design (N1), and portability across agents is Non-Goal 4, so a portability attribute would contradict a stated boundary rather than fill a gap. Adding any of these would be answering a question nobody asked. Testability is likewise not listed as a requirement, because it is what makes the verification column above possible rather than a property of the system I would defend to a reader. The fact under it is still load-bearing for every status in that column: CI runs the full suite with no `ANTHROPIC_API_KEY` and no network, so each "built" is an assertion that ran without me.
