...
4. **The accumulated catalog decays instead of compounding** (impact: medium, probability: high). The catalog is meant to gain value as it grows, but without active curation, growth degrades it: (1) the same preference is captured across sessions in slightly different words, so several near-duplicates coexist and none is authoritative; (2) an entry stops applying but is never retired, so it keeps surfacing or enforcing against a situation that no longer exists; (3) the always-loaded core grows without bound as globals accumulate, pushing standing context past the size where adherence holds; (4) keyword retrieval degrades at scale, missing paraphrased entries that share no literal term with the query while extra entries add noise, so a relevant convention silently fails to surface. Where risk 3 is one convention losing out within a single session, this is the corpus as a whole decaying as it accumulates, which makes that failure steadily more frequent and harder to diagnose.
   Example: over months I have corrected test placement several times in slightly different words ("colocate tests", "tests live next to the source", "no separate `tests/` tree"). Each became its own entry. Three near-duplicate conventions now sit in the catalog, one written for a repo layout I have since abandoned; when the agent scaffolds a new test, retrieval surfaces the stale one, and I cannot tell which of the three is authoritative.
   ...
   *Mitigations:*
   ...
   - **Budgeted always-on core:** the always-loaded set is capped and everything else is scoped, so accumulation cannot silently push standing context past the range where adherence holds. [R1.4, R1.5]
   - **Relevance-scoped injection:** entries load only where their scope matches, and stored knowledge is injected by relevance at prompt time, so what reaches context stays bounded as the catalog grows instead of scaling with it. [R1.4, R2.2]
   - **Retrieval escalates when keyword search is outgrown:** relevance retrieval is keyword-first today, with a Recall@k eval on my own corpus as the gate for adding an embedding index, so paraphrase misses at scale trigger a concrete upgrade rather than silent tolerance. [R2.2]
   - **Provenance as the audit substrate:** every entry records its origin and every enforcement is logged, so a duplicate, a stale entry, or a silent miss is traceable, and the governance passes above read those origins and fire-counts to decide what to retire. [N6]
