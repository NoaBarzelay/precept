...
2. **Mis-enforcement: a hard rule blocks correct work** (impact: high, probability: medium). A kept correction compiles to a deterministic block, so precision is what matters: an over-scoped, stale, or brittle rule blocks legitimate work, and repeated false blocks erode my trust in enforcement. *Mitigation:* a false block disables only the offending rule, never enforcement as a whole; a new HARD rule is shadow-run in log-only mode before it can block, and auto-demoted to SOFT on a false block (R3.3), with a one-command override. Status: partially mitigated (validated matchers and per-rule scope are built; shadow-arming and auto-demotion are designed).

3. **Non-deterministic retrieval means intended behaviors are not reliably applied** (impact: medium, probability: high). A soft convention steers only if retrieval surfaces it and the model then follows it, both probabilistic and worse as context grows, so a kept behavior can silently not fire. *Mitigation:* the real guarantee is R4, promote a must-hold behavior to a HARD hook that blocks without the model reading context (N2); the soft majority gets relevance-ranked injection and bounded accretion (R3.3), which lowers but cannot remove the miss. Status: open (the HARD-tier guarantee is built; the soft-side improvements are designed).

4. **Accumulated conventions and notes get hard to track and retrieve** (impact: medium, probability: high). On the soft plane, the catalog gathers duplicates and stale entries, the always-loaded set bloats context and degrades adherence, and keyword retrieval misses paraphrased notes, so a relevant convention silently fails to surface. *Mitigation:* governance decay, supersede, and conflict detection (R3.3), with provenance and the decision log as the audit substrate (N6); designed, an enforced budget on the always-on set and just-in-time injection. Status: partially mitigated; paraphrase recall waits on semantic retrieval (Open Question).

5. **The learning loop cannot reach a model and silently stops learning** (impact: medium, probability: high). If the backend has no credential (an expired subscription token once left it inert for weeks), hits the shared quota, or the provider is down, no candidates are produced, indistinguishable from a detector that found nothing; enforcement is unaffected (N2). *Mitigation:* a per-backend last-success timestamp that alarms on the absence of success, so a dead backend surfaces in one session, not weeks, and the loop fails open so a session is never blocked (N1). Status: partially mitigated; no fallback on the subscription-only path.

6. **The Claude Code contract changes underneath Precept** (impact: medium, probability: medium). Precept rides Anthropic's hook, exit-code, and permission surface, which is unversioned and has changed before; because the runtime fails open (N1), a change can silently downgrade a HARD rule to a no-op, so the real danger is not the fix (localized and small) but not noticing enforcement stopped. *Mitigation:* a startup and version-triggered self-check that asserts a known-bad action is still actually blocked and surfaces a failure loudly, so a break is caught and fixed quickly rather than running silent. Status: partially mitigated (a reachability check is built; the behavioral self-check is designed).

7. **The system becomes too expensive or too slow** (impact: medium, probability: medium). The loop's model calls cost money on the API backend and burn shared subscription quota on the CLI backend, and a Stop-gate verdict runs synchronously in the turn; too slow or costly and I stop running it, which halts the compounding. *Mitigation:* the enforcement hot path is model-free (N2), verdicts are consolidated to one relevance-gated call per turn, Haiku serves the cheap paths and Sonnet only compile, and a token meter prices per-flow spend; designed, detection moved fully async off the interactive path. Status: mitigated on latency, accepted cost debt.
